...
type Switcher10 struct {
	value    atomic.Value
	filePath string
	writeMu  sync.Mutex
}

var globalSwitcher10 *Switcher10
...
func Init(bp *coremain.BP, args any) (any, error) {
	cfg := args.(*Args)
	sw := &Switcher10{filePath: cfg.InitialValue}
...
func (s *Switcher10) Exec(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	return next.ExecNext(ctx, qCtx)
}

func (s *Switcher10) Api() *chi.Mux {
	r := chi.NewRouter()
...
func QuickSetup(_ sequence.BQ, raw string) (sequence.Matcher, error) {
	...
	return &switchMatcher10{expected: expected}, nil
}

type switchMatcher10 struct {
	expected string
}

func (m *switchMatcher10) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher10 == nil { return false, nil }
	currentVal := globalSwitcher10.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher10) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher10 == nil { return false }
		v := globalSwitcher10.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher10) GetValue() string {
