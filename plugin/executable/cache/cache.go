...
func NewCache(args *Args, opts Opts) *Cache {
	...
	p := &Cache{
		...
		logger:          logger,
		backend:         backend,
		rustMirrorGo:    isRustGoMirrorEnabled(),
	...
	if bridge, ok := initRustBridge(args, logger); ok {
		p.rustBridge = bridge
		logger.Info("initialized rust cache runtime bridge", zap.String("runtime", bridge.Name()))
	}
...
func (c *Cache) Exec(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	...
	if c.rustBridge != nil {
		return c.execWithRust(ctx, qCtx, next)
	}

	return c.execWithGoBackend(ctx, qCtx, next)
}

func (c *Cache) execWithGoBackend(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
...
func (c *Cache) execWithRust(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	q := qCtx.Q()
	...
	// Rust mode also leverages local L1 hot cache to avoid cgo crossing on repeated hits.
	kStr := *(*string)(unsafe.Pointer(&msgKeyBuf))
	k := key(kStr)
	h := k.Sum()
	shard := c.shards[h%shardCount]
	...
	lookup, err := c.rustBridge.LookupByKey(msgKeyBuf, now)
	if err != nil {
		keyBufferPool.Put(bufPtr)
		c.logger.Warn("rust cache lookup failed, fallback to go path", zap.Error(err))
		return c.execWithGoFallback(ctx, qCtx, next)
	}

	if lookup.State == bridgeLookupLazy {
		msgKey := string(msgKeyBuf)
		queryKeyCopy := append([]byte(nil), msgKeyBuf...)
		c.lazyHitTotal.Inc()
		c.doLazyUpdateRust(queryKeyCopy, msgKey, qCtx, next)
	}
	...
	c.tryStoreRustCache(msgKeyBuf, "", qCtx)
	keyBufferPool.Put(bufPtr)
	return err
}

func (c *Cache) execWithGoFallback(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	return c.execWithGoBackend(ctx, qCtx, next)
}

func (c *Cache) tryStoreRustCache(queryKey []byte, msgKey string, qCtx *query_context.Context) {
	r := qCtx.R()
	if r == nil {
		return
	}

	responseBytes, err := copyNoOpt(r).Pack()
	if err != nil {
		return
	}
	...
	stored, err := c.rustBridge.StoreByKey(queryKey, responseBytes, domainSet, time.Now())
	if err != nil {
		c.logger.Debug("rust cache store failed", zap.Error(err))
		return
	}
	if stored {
		c.updatedKey.Add(1)
	...
	if saveRespToCache(msgKey, qCtx, c.backend, c.args.LazyCacheTTL) {
		...
		h := k.Sum()
		shard := c.shards[h%shardCount]
		shard.updateL1(k, r, time.Now(), time.Now().Add(time.Duration(minTTL)*time.Second), domainSet)
	}
}

func (c *Cache) doLazyUpdateRust(queryKey []byte, msgKey string, qCtx *query_context.Context, next sequence.ChainWalker) {
	qCtxCopy := qCtx.Copy()
	queryKeyCopy := append([]byte(nil), queryKey...)
	...
	lazyUpdateFunc := func() (any, error) {
		...
		err := next.ExecNext(ctx, qCtx)
		if err != nil && err != sequence.ErrExit {
			c.logger.Warn("failed to update lazy rust cache", qCtx.InfoField(), zap.Error(err))
			return nil, nil
		}

		c.tryStoreRustCache(queryKeyCopy, msgKey, qCtx)
		return nil, nil
	}
...
func (c *Cache) Close() error {
	...
	if c.rustBridge != nil {
		if err := c.rustBridge.Close(); err != nil {
			c.logger.Warn("failed to close rust cache runtime bridge", zap.Error(err))
		}
	}
	c.closeOnce.Do(func() {
		close(c.closeNotify)
...
func (c *Cache) loadDump() error {
	...
	if c.rustBridge != nil {
		...
		if err := c.rustBridge.ImportDump(payload); err != nil {
			return err
		}
		if c.rustMirrorGo {
		...
		c.logger.Info("cache dump loaded into rust runtime", zap.Int("bytes", len(payload)))
		return nil
	}
...
func (c *Cache) dumpCache() error {
	...
	if c.rustBridge != nil {
		dumpBytes, err := c.rustBridge.ExportDump()
		if err != nil {
			return err
		}
		if _, err := f.Write(dumpBytes); err != nil {
			return err
		}
		c.logger.Info("rust cache dumped", zap.Int("bytes", len(dumpBytes)))
		return nil
	}
...
func (c *Cache) Api() *chi.Mux {
	...
	r.Get("/dump", coremain.WithAsyncGC(func(w http.ResponseWriter, req *http.Request) {
		...
		if c.rustBridge != nil {
			...
			if _, err := w.Write(dumpBytes); err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			return
	...
	r.Get("/show", coremain.WithAsyncGC(func(w http.ResponseWriter, req *http.Request) {
		...
		if c.rustBridge != nil {
			...
			if err := c.renderShowFromDump(w, dumpBytes, query, limit, offset); err != nil {
				c.logger.Error("failed to enumerate rust cache dump", zap.Error(err))
			}
			return
		}
...
func (c *Cache) renderShowFromDump(w io.Writer, dumpPayload []byte, query string, limit, offset int) error {
	isIPLike := strings.Contains(query, ".") || strings.Contains(query, ":")
	now := time.Now()
	matchedCount := 0
	sentCount := 0
	...
	reusableMsg := new(dns.Msg)
	err := forEachDumpEntry(bytes.NewReader(dumpPayload), func(entry *CachedEntry) error {
		cacheExpirationTime := time.Unix(entry.GetCacheExpirationTime(), 0)
		if cacheExpirationTime.Before(now) {
			return nil
		}

		keyStr := keyToString(key(entry.GetKey()))
		found := query == "" || strings.Contains(strings.ToLower(keyStr), query)
		msgDecoded := false

		if !found && isIPLike {
			if err := reusableMsg.Unpack(entry.GetMsg()); err == nil {
				msgDecoded = true
				for _, rr := range reusableMsg.Answer {
					if strings.Contains(rr.String(), query) {
						found = true
		...
		fmt.Fprintf(w, "----- Cache Entry -----\n")
		fmt.Fprintf(w, "Key:           %s\n", keyStr)
		if entry.GetDomainSet() != "" {
			fmt.Fprintf(w, "DomainSet:     %s\n", entry.GetDomainSet())
		}
		fmt.Fprintf(w, "StoredTime:    %s\n", time.Unix(entry.GetMsgStoredTime(), 0).Format(time.RFC3339))
		fmt.Fprintf(w, "MsgExpire:     %s\n", time.Unix(entry.GetMsgExpirationTime(), 0).Format(time.RFC3339))
		fmt.Fprintf(w, "CacheExpire:   %s\n", cacheExpirationTime.Format(time.RFC3339))
...
func (c *Cache) Flush() error {
	if c.rustBridge != nil {
		if err := c.rustBridge.Flush(); err != nil {
			c.logger.Error("failed to flush rust cache runtime", zap.Error(err))
			return err
		}
	}
...
func forEachDumpEntry(r io.Reader, visitor func(*CachedEntry) error) error {
	gr, err := gzip.NewReader(r)
	if err != nil {
	...
	if gr.Name != dumpHeader {
		return fmt.Errorf("invalid or old cache dump, header is %s, want %s", gr.Name, dumpHeader)
	}
