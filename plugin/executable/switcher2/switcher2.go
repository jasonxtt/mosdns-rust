...
func Init(bp *coremain.BP, args any) (any, error) {
	cfg := args.(*Args)
	sw := &Switcher2{filePath: cfg.InitialValue}
...
func (m *switchMatcher2) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher2 == nil { return false, nil }
	currentVal := globalSwitcher2.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher2) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher2 == nil { return false }
		v := globalSwitcher2.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher2) GetValue() string {
