...
type Switcher14 struct {
	value    atomic.Value
	filePath string
	writeMu  sync.Mutex
}

var globalSwitcher14 *Switcher14
...
func Init(bp *coremain.BP, args any) (any, error) {
	cfg := args.(*Args)
	sw := &Switcher14{filePath: cfg.InitialValue}
...
func (s *Switcher14) Exec(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	return next.ExecNext(ctx, qCtx)
}

func (s *Switcher14) Api() *chi.Mux {
	r := chi.NewRouter()
...
func QuickSetup(_ sequence.BQ, raw string) (sequence.Matcher, error) {
	...
	return &switchMatcher14{expected: expected}, nil
}

type switchMatcher14 struct {
	expected string
}

func (m *switchMatcher14) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher14 == nil { return false, nil }
	currentVal := globalSwitcher14.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher14) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher14 == nil { return false }
		v := globalSwitcher14.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher14) GetValue() string {
