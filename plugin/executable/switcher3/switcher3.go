...
type Switcher3 struct {
	value    atomic.Value
	filePath string
	writeMu  sync.Mutex
}

var globalSwitcher3 *Switcher3
...
func Init(bp *coremain.BP, args any) (any, error) {
	cfg := args.(*Args)
	sw := &Switcher3{filePath: cfg.InitialValue}
...
func (s *Switcher3) Exec(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	return next.ExecNext(ctx, qCtx)
}

func (s *Switcher3) Api() *chi.Mux {
	r := chi.NewRouter()
...
func QuickSetup(_ sequence.BQ, raw string) (sequence.Matcher, error) {
	...
	return &switchMatcher3{expected: expected}, nil
}

type switchMatcher3 struct {
	expected string
}

func (m *switchMatcher3) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher3 == nil { return false, nil }
	currentVal := globalSwitcher3.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher3) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher3 == nil { return false }
		v := globalSwitcher3.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher3) GetValue() string {
