...
type Switcher11 struct {
	value    atomic.Value
	filePath string
	writeMu  sync.Mutex
}

var globalSwitcher11 *Switcher11
...
func Init(bp *coremain.BP, args any) (any, error) {
	cfg := args.(*Args)
	sw := &Switcher11{filePath: cfg.InitialValue}
...
func (s *Switcher11) Exec(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	return next.ExecNext(ctx, qCtx)
}

func (s *Switcher11) Api() *chi.Mux {
	r := chi.NewRouter()
...
func QuickSetup(_ sequence.BQ, raw string) (sequence.Matcher, error) {
	...
	return &switchMatcher11{expected: expected}, nil
}

type switchMatcher11 struct {
	expected string
}

func (m *switchMatcher11) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher11 == nil { return false, nil }
	currentVal := globalSwitcher11.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher11) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher11 == nil { return false }
		v := globalSwitcher11.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher11) GetValue() string {
