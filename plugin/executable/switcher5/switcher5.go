...
func (m *switchMatcher5) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher5 == nil { return false, nil }
	currentVal := globalSwitcher5.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher5) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher5 == nil { return false }
		v := globalSwitcher5.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher5) GetValue() string {
