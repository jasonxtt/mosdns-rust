...
func Init(bp *coremain.BP, args any) (any, error) {
	cfg := args.(*Args)
	sw := &Switcher16{filePath: cfg.InitialValue}
...
func (s *Switcher16) Exec(ctx context.Context, qCtx *query_context.Context, next sequence.ChainWalker) error {
	return next.ExecNext(ctx, qCtx)
}

func (s *Switcher16) Api() *chi.Mux {
	r := chi.NewRouter()
...
func QuickSetup(_ sequence.BQ, raw string) (sequence.Matcher, error) {
	...
	return &switchMatcher16{expected: expected}, nil
}

type switchMatcher16 struct {
	expected string
}

func (m *switchMatcher16) Match(_ context.Context, qCtx *query_context.Context) (bool, error) {
	if m.expected == "A" {
		return qCtx.HasFastFlag(switchBit), nil
	}
	// Fallback for 'B' or other non-A expectations
	if globalSwitcher16 == nil { return false, nil }
	currentVal := globalSwitcher16.value.Load().(string)
	return currentVal == m.expected, nil
}

func (m *switchMatcher16) GetFastCheck() func(qCtx *query_context.Context) bool {
	exp := m.expected
	return func(qCtx *query_context.Context) bool {
		if exp == "A" {
			return qCtx.HasFastFlag(switchBit)
		}
		if globalSwitcher16 == nil { return false }
		v := globalSwitcher16.value.Load().(string)
		return v == exp
	}
}

func (s *Switcher16) GetValue() string {
